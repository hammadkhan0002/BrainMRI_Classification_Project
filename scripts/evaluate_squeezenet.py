"""
Evaluate the saved SqueezeNetTransfer model on the Brain MRI test dataset.

This script loads the best SqueezeNetTransfer checkpoint and computes standard
...
from models.squeezenet_transfer import SqueezeNetTransfer
from utils.dataset import get_data_loaders
from utils.trainer import get_default_device


CHECKPOINT_PATH = Path("results/models/squeezenet_best.pth")
EVALUATION_OUTPUT_PATH = Path("results/evaluation/squeezenet_evaluation.txt")
BATCH_SIZE = 32
...
def load_model(checkpoint, device):
    """Create SqueezeNetTransfer and load the saved model weights."""
...
def format_metrics(metrics, class_names):
    ...
    lines = [
        "SqueezeNetTransfer Test Evaluation",
        "=" * 20,
...
def main():
    """Evaluate the saved SqueezeNetTransfer model on the test dataset."""
    try:
